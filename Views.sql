...
AS
SELECT B.BRANCH_ID AS BRANCH_ID, B.NAME AS BRANCH_NAME, COUNT(A.ACC_NO), SUM(A.BALANCE) AS TOTAL_BAL 
...
AS
SELECT TRANS_ID, ACC_NO, TRANS_TYPE, TRANS_DATE, AMOUNT FROM DB_ADMIN.TRANSACTION
WHERE TRUNC(TRANS_DATE) > TRUNC(SYSDATE)-7 AND DEBIT_CREDIT_FLAG = 0;
...
AS
SELECT TRANS_ID, ACC_NO, TRANS_TYPE, TRANS_DATE, AMOUNT FROM DB_ADMIN.TRANSACTION
WHERE TRUNC(TRANS_DATE) > TRUNC(SYSDATE)-7 AND DEBIT_CREDIT_FLAG = 1;
